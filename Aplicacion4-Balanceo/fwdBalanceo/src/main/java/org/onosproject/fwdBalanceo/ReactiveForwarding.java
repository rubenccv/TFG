 ...
 */
package org.onosproject.fwdBalanceo;
...
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.FLOW_PRIORITY;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.FLOW_PRIORITY_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.FLOW_TIMEOUT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.FLOW_TIMEOUT_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.IGNORE_IPV4_MCAST_PACKETS;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.IGNORE_IPV4_MCAST_PACKETS_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.IPV6_FORWARDING;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.IPV6_FORWARDING_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_DST_MAC_ONLY;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_DST_MAC_ONLY_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_ICMP_FIELDS;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_ICMP_FIELDS_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_IPV4_ADDRESS;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_IPV4_ADDRESS_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_IPV4_DSCP;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_IPV4_DSCP_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_IPV6_ADDRESS;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_IPV6_ADDRESS_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_IPV6_FLOW_LABEL;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_IPV6_FLOW_LABEL_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_TCP_UDP_PORTS;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_TCP_UDP_PORTS_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_VLAN_ID;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.MATCH_VLAN_ID_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.PACKET_OUT_OFPP_TABLE;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.PACKET_OUT_OFPP_TABLE_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.PACKET_OUT_ONLY;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.PACKET_OUT_ONLY_DEFAULT;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.RECORD_METRICS;
import static org.onosproject.fwdBalanceo.OsgiPropertyConstants.RECORD_METRICS_DEFAULT;
...
public class ReactiveForwarding {
	...
	public void activate(ComponentContext context) {
		...
		log.info("Activada aplicacion fwdBalanceo", appId.id());
		...
		/*Creamos el grupo al que le ponemos que cuando se mande por el, él tratamiento sea enviarlo por 
		el puerto1*/ 
		for(Device d:devices) {
			...
			DefaultGroupDescription degd = new DefaultGroupDescription(d.id(), GroupDescription.Type.SELECT,
					gbs,k,137,appId);
			...
			public void run() {
				...
				for (Device d: devices) {
					...
					for (Port p: ports) {		
						...
						//Y sumamos el trafico total que esta habiendo por la red en bps
						if(traffic!=null)
					...
					/*Si los bits por segundo superan ciertos umbrales vamos aumentando los puertos 
                		por los cuales mandar el trafico.

                		Con este codigo ya tenemos implementado el grupo, falta decirle cuando actuar
					 */
					if(bitsPorSegundo>=20000) { //100kbps
					...
					if(bitsPorSegundo>=30000) { //20Mbps
...
//			 * envien por todos los puertos menos los troncales (que se enviara por el grupo)
//			 * y que se instalara en el switch
...
//					.matchEthType(EthType.ARP.value())
//					.build();
//			
//			TrafficTreatment trtr2 = 
//					DefaultTrafficTreatment.builder()
...
//			log.info("Creada regla para el trafico broadcast");
//			//Una vez creada la regla le decimos al paquete que vuelva al switch a la tabla 1
//			TrafficTreatment sendBack = DefaultTrafficTreatment.builder().transition(1)
...
//
//
//				log.info("Enviado paquete por el grupo");
//
//				TrafficTreatment sendGroup = DefaultTrafficTreatment.builder()
//						.setOutput(PortNumber.TABLE).build();
//				context.treatmentBuilder().addTreatment(sendGroup);
//			}
//			else {
//				log.info("Enviado paquete por el puerto correspondiente");
//				//Duda: ¿portNumber no es FLOOD? Por que enviarlo por ahi? --> Se le llama desde otro sitio donde el portNumber es el puerto destino
//				/*TODO Duda2: Al hacer captura sobre el puerto cada poco envia 
//            	ARP preguntando la MAC, por que? Secuencia de 6seg 22seg 6seg 22seg....*/
//				context.treatmentBuilder().setOutput(portNumber);
//			}
//		}
//		context.send();
	...
	private void installRule(PacketContext context, PortNumber portNumber, ReactiveForwardMetrics macMetrics) {
       ...
       De esta forma tenemos implementado el balanceo*/
		...
		if(portNumber.toLong()==puerto1 || portNumber.toLong()==puerto2 || 
			...
			log.info("Regla creada para el trafico que va por los puertos troncales");
		...
		//Si el trafico va por otros puertos hacemos que se mande por ese unicamente
		else {
			TrafficTreatment treatment = DefaultTrafficTreatment.builder()
					.setOutput(portNumber)
					.build();

			log.info("Regla creada para el trafico de los puertos normales");
			ForwardingObjective forwardingObjective = DefaultForwardingObjective.builder()
					.withSelector(selectorBuilder.build())
					.withTreatment(treatment)
					.withPriority(40500)
